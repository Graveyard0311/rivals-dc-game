...
function refreshTeamUp() {
  ...
  const teamHeroIds = fighters
    .filter(f => f.userData.team === player.userData.team)
    .map(f => f.userData.hero.id);
...
function teamUpParticipants() {
  ...
  return fighters.filter(f =>
    f.userData.team === player.userData.team &&
    activeTeamUp.members.includes(f.userData.hero.id)
  );
}
...
function useTeamUp() {
  ...
  if (activeTeamUp.effectKind === 'teamShield') {
    allies
      .filter(f => f.position.distanceTo(player.position) <= radius)
      .forEach(f => applyEffect(f, 'shield', { duration: activeTeamUp.duration, actor: player }));
  }

  if (activeTeamUp.effectKind === 'duoHasteShield') {
    participants.forEach(f => {
      applyEffect(f, 'haste', { duration: activeTeamUp.duration, actor: player });
  ...
  if (activeTeamUp.effectKind === 'teamHeal') {
    allies
      .filter(f => f.position.distanceTo(player.position) <= radius)
      .forEach(f => {
  ...
  if (activeTeamUp.effectKind === 'enemyDisrupt') {
    enemies
      .filter(f => f.position.distanceTo(player.position) <= radius)
      .forEach(f => {
